...
VALID_WORKLOADS = ["basic", "cockroach", "remastering", "tpcc", "movr", "pps", "movie", "dsh", "smallbank", 'benchx']

get_container_data_dir = lambda user: f"/home/{user}/data"
get_host_data_dir = lambda user: f"/home/{user}/data"
get_slog_container_name = lambda user: f"{user}_slog"
get_slog_benchmark_container_name = lambda user: f"{user}_benchmark"
get_slog_client_container_name = lambda user: f"{user}_slog_client"
get_slog_data_mount = lambda user: docker.types.Mount(target=get_container_data_dir(user), source=get_host_data_dir(user), type="bind")
get_benchmark_container_name = lambda user: f"{user}_benchmark"
...
def fetch_data(machines, user, tag, out_path):
    ...
    for m in machines:
        addr = m["address"]
        data_path = os.path.join(get_host_data_dir(user), tag)
        data_tar_file = f'{m["name"]}.tar.gz'
        data_tar_path = os.path.join(get_host_data_dir(user), data_tar_file)
...
class AdminCommand(Command):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--image", default=DEFAULT_SLOG_IMG, help="Name of the Docker image to use")
        parser.add_argument("--user", "-u", default=DEFAULT_USER, help="Username of the target machines")
    ...
    def pull_slog_image(self, args):
        ...
        LOG.info('Remote procs are: "%s"', self.remote_procs)
...
class StartCommand(AdminCommand):
    ...
    def do_command(self, args):
        ...
        config_path = os.path.join(get_container_data_dir(args.user), self.config_name)
        ...
        def clean_up(remote_proc):
            ...
            cleanup_container(client, get_slog_container_name(args.user), addr=addr)
        ...
        def start_container(remote_proc):
            ...
            shell_cmd = (f"{args.bin} " f"--config {config_path} " f"--address {priv_address} " f"--data-dir {get_container_data_dir(args.user)} ")
            client.containers.run(
                args.image,
                name=get_slog_container_name(args.user),
                command=["/bin/sh", "-c", f"{sync_config_cmd} && {shell_cmd}"],
                # Mount a directory on the host into the container
                mounts=[get_slog_data_mount(args.user)],
                # Expose all ports from container to host
                network_mode="host",
                # Avoid hanging this tool after starting the server
                detach=True,
                environment=parse_envs(args.e),
                # CPU and DRAM restrictions corresponding to a 'r5.4xlarge' machine
    ...
    def do_command(self, args):
        ...
        def stop_container(remote_proc):
            try:
                ...
                c = remote_proc.docker_client.containers.get(get_slog_container_name(args.user))
    ...
    def do_command(self, args):
        ...
        for reg, g in itertools.groupby(remote_procs, key_func):
            ...
            for client, addr, _, _, part, *_ in g:
                status = get_container_status(client, get_slog_container_name(args.user))
...
class LogsCommand(AdminCommand):
    ...
    def do_command(self, args):
        ...
        if args.container is not None:
            ...
            container = get_slog_benchmark_container_name(args.user)
        else:
            container = get_slog_container_name(args.user)
        try:
        ...
        for r, reg in enumerate(self.config.regions):
            for p, (pub_addr, priv_addr) in enumerate(
                ...
                shell_cmd = (
                    ...
                    f"--address {priv_addr} "
                    f"--data-dir {get_container_data_dir(args.user)} "
                )
                container_name = f"slog_{r}_{p}"
...
class BenchmarkCommand(AdminCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("-d",  "--duration", type=int, default=0, help="How long the benchmark is run in seconds")
        parser.add_argument(       "--tag", help="Tag of this benchmark run. Auto-generated if not provided")
        parser.add_argument("-wl", "--workload", choices=VALID_WORKLOADS, default="basic", help="Name of the workload to run benchmark with")
        ...
        parser.add_argument(       "--clients", type=str, default="0", help="Number of clients sending synchronized txns")
        ...
        parser.add_argument("-db", "--database", type=str, default="detock", help="Name of the database to use")
    ...
    def do_command(self, args):
        ...
        if args.out_dir != "":
            ...
            out_dir = os.path.join(get_container_data_dir(args.user), tag)

        # Clean up everything
        def clean_up(remote_proc):
            client, addr, *_ = remote_proc
            cleanup_container(client, get_benchmark_container_name(args.user), addr=addr)
        ...
        def parse_clients_num(clients: str) -> List[int]:
            return [int(c) for c in clients.split(",")]
        ...
        def benchmark_runner(enumerated_proc):
            ...
            # The number of clients can be different per region (or per replica for Calvin)
            clients_num_per_region = parse_clients_num(args.clients)
            idx = rep if args.database == "calvin" else reg
            #LOG.info("%s: Client counts are : %s, we will use idx: %s", addr, clients_num_per_region, idx)
            clients_num = clients_num_per_region[idx] if idx < len(clients_num_per_region) else clients_num_per_region[-1]
            shell_cmd = (
                f"benchmark "
                f"--config {config_path} "
                f"--region {reg} "
            ...
            container = client.containers.run(
                args.image,
                name=f"{get_benchmark_container_name(args.user)}",
...
class CollectServerCommand(AdminCommand):
    ...
    def do_command(self, args):
        if not args.download_only:
            ...
            out_dir = os.path.join(get_host_data_dir(args.user), args.tag)
            config_path = os.path.join(get_host_data_dir(args.user), self.config_name)
            rmdir_cmd = f"rm -rf {out_dir}"
            mkdir_cmd = f"mkdir -p {out_dir}"
            ...
            for addr in addresses:
                cmd = f'{SSH} {args.user}@{addr} "{cp_config_cmd}"'
                commands.append(f"({cmd}) & ")
            ...
            def trigger_flushing_metrics(enumerated_address):
                ...
                out_dir = os.path.join(get_container_data_dir(args.user), args.tag)
                container_name = f"{get_slog_client_container_name(args.user)}_{i}"
                ...
                docker_client.containers.run(args.image, name=f"{get_slog_client_container_name(args.user)}_{i}", command=run_cmd, remove=True)
