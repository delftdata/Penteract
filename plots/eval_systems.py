...
WORKLOAD_CAPITALIZATION = {'ycsb': 'YCSB', 'tpcc': 'TPC-C', 'movr': 'MovR', 'movie': 'DS Movie', 'pps': 'PPS', 'dsh': 'DS Hotels', 'smallbank': 'SmallBank', 'benchx': 'BenchX'}
BAR_WIDTH = 0.18
AVG_CROSS_REGION_LATENCY = 111.3 # In ms, used for the x-axis of the network experiments when plotting relative numbers
...
def make_plot(plot='baseline', workload='ycsb', env='st', latency_percentiles=[50, 95, 99], skip_aborts=False, separate_latencies=False, log_latencies=True, costs_per_txn=True, relative_numbers=False):
    ...
    if relative_numbers:
        if plot == 'network':
            default_x_point = AVG_CROSS_REGION_LATENCY
            x_lab = 'Avg cross-region latency (ms)'
            data['x_var'] = data['x_var'] + AVG_CROSS_REGION_LATENCY
        elif plot == 'packet_loss':
            default_x_point = 0
        for col in data.columns:
            if 'x_var' not in col:
                data[col] = data[col] / data[data['x_var'] == default_x_point][col].values[0]

    # Extract data
    xaxis_points = data['x_var']
    # For some experiments, we have to adjust the x_values
    if (workload == 'tpcc' or workload == 'benchx') and plot == 'baseline':
        if env == 'st':
            ...
            included_rows = [True if x in included_tpcc_xaxis_points else False for x in data['x_var']]
            data = data[included_rows].reset_index(drop=True)
        else:
    ...
    if workload == 'smallbank' and plot == 'baseline':
        xaxis_points = [x * 0.2 for x in xaxis_points] # Only 20% of txns can be MH in SmallBank
        data['x_axis_points'] = xaxis_points
        xaxis_points = data['x_axis_points']
    ...
    if not skip_aborts:
        ...
        y_labels = ['Throughput (txn/s)', 'Latency (ms)', 'Aborts (%)', 'Data transfers (GB/s)', 'Hourly Cost ($/h)']
        y_labels_crdb = ['Throughput CRDB', 'Latency CRDB', 'Aborts CRDB', 'Data transfers CRDB', 'Hourly Cost CRDB']
        ...
        y_labels = ['Throughput\n(txn/s)', 'Latency (ms)', 'Data transfers\n(GB/s)', 'Hourly Cost ($/h)']
        y_labels_crdb = ['Throughput\nCRDB', 'Latency\nCRDB', 'Data transfers\nCRDB', 'Hourly Cost CRDB']
        if separate_latencies:
    ...
    if relative_numbers:
        y_labels = ['Throughput\n(relative)', 'Latency (relative)', 'Data transfers\n(relative)', 'Cost per 10k\ntxns (relative)']

    databases = ['Calvin', 'SLOG', 'Detock', 'Janus', 'CockroachDB']
    databases = [db for db in databases if f"{db.lower()}_throughput" in data.columns]
    ...
    if env == 'aws':
        fig, axes = plt.subplots(1, len(metrics), figsize=(15, 1.9), sharex=True)
    else:
    ...
    for ax, metric, y_label, subplot_title, crdb_ylabel in zip(axes, metrics, y_labels, subplot_titles, y_labels_crdb):
        ...
        if metric != 'latency' and not relative_numbers:  # Only create twin axis for non-latency metrics and when not plotting relative numbers
            ax_twin = ax.twinx()
        ...
        for db, color, style in zip(databases, colors, line_styles):
            ...
            current_ax = ax_twin if (db == 'CockroachDB' and not relative_numbers) else ax
            if plot == 'scalability':
                column_name = f'{db}_input_throughput'
                if column_name.lower() in data.columns:
                    xaxis_points = data[column_name.lower()]
            elif plot == 'sunflower':
                column_name = f'{db.lower()}_throughput'
                xaxis_points = 24 * data[data[column_name.lower()].notnull()]['x_var'] / max(data[data[column_name.lower()].notnull()]['x_var'])
            if metric != 'latency':
                column_name = f'{db}_{metric}'
                if column_name.lower() in data.columns:  # Plot only if the column exists in the CSV
                    if metric == 'bytes' and not relative_numbers:
                    ...
                    if db == 'CockroachDB' and not relative_numbers:
        ...
        ax.grid(True)
        if metric != 'latency' and not relative_numbers:
            ax_twin.set_ylim(bottom=0)
            ax_twin.set_ylabel(crdb_ylabel)
        if relative_numbers and plot == 'network':
            ax.axvline(x=AVG_CROSS_REGION_LATENCY, color='gray', linestyle='--', linewidth=1.5, alpha=0.8, zorder=1)
            #ax.text(AVG_CROSS_REGION_LATENCY + 10, 0.95, 'default geo-distr.\nconfiguration', rotation=0, verticalalignment='top', fontsize=7, color='gray', transform=ax.get_xaxis_transform())
        if workload == 'ycsb' and not relative_numbers:
            if plot == 'baseline':
                ax.set_xticks(np.linspace(0, 100, 6))  # 0%, 20%, ..., 100%
                ax.set_xlim(0, 100)
                ax.minorticks_off()
                if metric == 'throughput' and not relative_numbers:
                ...
                if metric == 'throughput' and not relative_numbers:
                    ax_twin.set_ylim(0, 10000)
        elif workload == 'tpcc' and not relative_numbers:
            if plot == 'baseline':
                ax.set_xticks(np.linspace(0, 100, 6))  # 0%, 20%, ..., 100%
                ax.set_xlim(0, 100)
                ax.minorticks_off()
                ...
                ax.set_xticks(np.linspace(0.0, 1.0, 6))  # 0.0, 0.2, ..., 1.0
                ax.set_xlim(0, 1)
                ax_twin.set_ylim(0, 1000)
            elif plot == 'scalability' and not relative_numbers:
            ...
            if plot == 'baseline':
                ax.set_xticks(np.linspace(0, 20, 6))  # 0%, 20%, ..., 100%
                ax.set_xlim(0, 20)
            elif plot == 'skew':
                ax.set_xticks(np.linspace(0.0, 1.0, 6))  # 0.0, 0.2, ..., 1.0
                ax.set_xlim(0, 1)
            ...
            if plot == 'baseline':
                ...
                if relative_numbers:
                    ax.set_xlim(left=0, right=500+AVG_CROSS_REGION_LATENCY)
                else:
                    ax.set_xlim(left=0, right=1000)
            elif plot == 'packet_loss':
                ax.set_xlim(0, 10)
        ...
        elif metric == 'latency' and relative_numbers:
            ax.set_ylim(bottom=0.1)  # For relative latency plots, we want to show the area below 1 to see improvements, but we can still remove extra whitespace below 0.5
        else:
        ...
        elif metric == 'throughput' and not relative_numbers:
    ...
    plt.savefig(png_path, dpi=300, bbox_inches='tight', pad_inches=0.025)
...
def make_bar_plot(plot='vary_hw', workload='ycsb', env='st', latency_percentiles=[50, 99], skip_aborts=True, separate_latencies=False, log_latencies=True, costs_per_txn=True):
    ...
    if env == 'aws':
        ...
        if len(metrics) == 5:
            ...
            wr = [1, 1.5, 1, 1]
        fig, axes = plt.subplots(1, len(metrics), width_ratios=wr, figsize=(15, 1.65), sharex=True)
    else:
...
def make_resource_util_plots(plot='baseline', workload='ycsb', env='st', relative_numbers=False):
    ...
    if plot == 'baseline':
        ...
        if relative_numbers:
            x_lab = 'Avg cross-region latency (ms)'
            x_lim = (0, 500+AVG_CROSS_REGION_LATENCY)
        else:
            x_lab = 'Extra delay (ms)'
            x_lim = (0,1000)
    elif plot == 'packet_loss':
    ...
    # Extract data
    xaxis_points = data['x_var']
    if plot == 'network' and env == 'aws':
        xaxis_points = [x + AVG_CROSS_REGION_LATENCY for x in xaxis_points]
        data['x_var'] = xaxis_points
        xaxis_points = data['x_var'] # This turns the xaxis_points into a pandas Series avoiding lindicies slicing issues
    if workload == 'tpcc' and plot == 'baseline':
        ...
        data['x_axis_points'] = xaxis_points
        xaxis_points = data['x_axis_points']
    elif workload == 'smallbank' and plot == 'baseline':
        xaxis_points = [x * 0.2 for x in xaxis_points] # Only 20% of txns can be MH in SmallBank
        data['x_axis_points'] = xaxis_points
        xaxis_points = data['x_axis_points']
    elif workload == 'movr' and plot == 'baseline':
    ...
    databases = ['Calvin', 'SLOG', 'Detock', 'Janus', 'CockroachDB']
    databases = [db for db in databases if f"{db.lower()}_cpu_usage_percent" in data.columns]
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:brown']
    metrics = ['cpu_usage_percent', 'mem_usage_percent', 'net_usage_percent', 'disk_usage_percent', 'aborts']
    subplot_titles = ['CPU Utilization', 'Memory Utilization', 'Network Utilization', 'Disk Utilization', 'Aborts']
    y_labels = [
        'CPU Utilization (%)',
        'Memory Utilization (%)',
        'Network Utilization (%)',
...
def make_txn_type_ablations(plot='vary_hw', workload='ycsb', env='st', log_latencies=True):
    ...
    fixed_percentile = '95'
    databases = ['Calvin', 'SLOG', 'Detock', 'Janus', 'CockroachDB']
    databases = [db for db in databases if f"{db.lower()}_lsh_p50" in data.columns]
    colors = ['tab:blue', 'tab:orange', 'tab:green', 'tab:red', 'tab:brown']
