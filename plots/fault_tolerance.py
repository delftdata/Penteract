...
if exp == 'combined':
    ...
    fig, axes = plt.subplots(1, 2, figsize=(8, 1.8), sharex=True)
    ...
    for ax, scenario in zip(axes, ['prime_node_failure', 'region_failure']):
        ...
        ax.set_title(scenario.replace('_', ' ').title(), fontsize=9)
    ...
    all_clients_tps_series = all_clients_df['txn_id'].resample(resolution).count() * 10
    # 5. Calculate the rolling average (e.g., over 2 surrounding seconds)
    all_clients_throughput_smooth = all_clients_tps_series.rolling(window=TPS_WINDOW_SIZE, center=True, min_periods=1).mean()
...
fig.legend(loc='upper center', bbox_to_anchor=(0.5, 1.1), ncol=3, fontsize=9)
plt.tight_layout()
