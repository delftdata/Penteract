...
parser.add_argument("-s",  "--scenario", default='sunflower', choices=VALID_SCENARIOS, help="Type of experiment scenario to analyze (default: baseline)")
parser.add_argument("-w",  "--workload", default='movie', choices=VALID_WORKLOADS, help="Workload to run (default: ycsb)")
...
parser.add_argument("-rn", "--relative_numbers", default=True, help="Whether or not to plot the network & packet loss relative to the baseline costs (instead of absolute costs).")
...
parser.add_argument("-ne", "--no_extraction", default=False, help="Whether to skip extraction and just run the actual plotting")
parser.add_argument("-md", "--merge_dependent", default=True, help="Whether to merge dependent transactions in the plots.")
...
for system in system_dirs:
    ...
    x_vals = [join(system, dir) for dir in os.listdir(system) if dir.replace('.', '').lstrip("-").isnumeric()]
    ...
    for x_val in x_vals:
        ...
        if system.split('/')[-1] == 'crdb' and workload == 'benchx' and throughputs[system.split('/')[-1]][x_val.split('/')[-1]] == 0:
            total_committed = 0
            for cl in csv_files[system.split('/')[-1]][x_val.split('/')[-1]]:
                total_committed += csv_files[system.split('/')[-1]][x_val.split('/')[-1]][cl]['summary']['committed'].sum()
            duration = 60
            try:
                with open(join(x_val, 'raw_logs', 'benchmark_cmd.log'), "r", encoding="utf-8") as f:
                    for l in f.read().split('\n'):
                        if ' --duration ' in l:
                            duration = int(l.split(' --duration ')[1].split(' ')[0])
                            break
            except Exception:
                pass
            throughputs[system.split('/')[-1]][x_val.split('/')[-1]] = total_committed / duration
            input_throughputs[system.split('/')[-1]][x_val.split('/')[-1]] = throughputs[system.split('/')[-1]][x_val.split('/')[-1]]
    ...
    for x_val in x_vals:
        ...
        for ip in log_files[system.split('/')[-1]][x_val.split('/')[-1]]['net_traffic_logs'].keys():
            ...
            if resource_util:
                ...
                lower_bound = timestamps[timestamps < tz_adjusted_start].max()
                upper_bound = timestamps[timestamps > tz_adjusted_end].min()
                filtered = res_log[(timestamps > lower_bound) & (timestamps < upper_bound)].copy()
...
for system in system_dirs:
    ...
    for x_val in x_vals:
        ...
        if system.split('/')[-1] == 'crdb' and workload == 'tpcc':
            ...
            for client in clients:
                client_txns = pd.read_csv(join(x_val, 'client', client, 'transactions.csv'))
                ...
                if 'code' in client_txns.columns:
                ...
                if 'janus' in system.lower() or 'calvin' in system.lower():
                    ...
                    if workload == "tpcc" and 'code' in client_txns.columns:
                ...
                if merge_dependent and 'code' in client_txns.columns:
                    client_txns['dep_id'] = client_txns['code'].astype(str).str.extract(r'dep_(\d+)')
                    independent = client_txns[client_txns['dep_id'].isna()].copy()
                    dependent = client_txns[client_txns['dep_id'].notna()].copy()
                    ...
                    if not dependent.empty:
                        ...
                        merged['duration'] = merged['received_at'] - merged['sent_at']
                        merged = merged.drop(columns=['dep_id'])
                        independent = independent.drop(columns=['dep_id'])
                        
                        excess_txns = len(dependent) - len(merged)
                        duration_sec = (end_timestamps[system.split('/')[-1]][x_val.split('/')[-1]] - start_timestamps[system.split('/')[-1]][x_val.split('/')[-1]]) / 1000.0
                        throughputs[system.split('/')[-1]][x_val.split('/')[-1]] -= excess_txns / duration_sec
                        
                        client_txns = pd.concat([independent, merged], ignore_index=True)
                    else:
                        client_txns = client_txns.drop(columns=['dep_id'])
                all_latencies.append(client_txns["duration"].to_numpy())
                lsh_latencies.append(client_txns[client_txns["lsh"]]["duration"].to_numpy())
                fsh_latencies.append(client_txns[client_txns["fsh"]]["duration"].to_numpy())
                mh_latencies.append(client_txns[client_txns["mh"]]["duration"].to_numpy())
                read_latencies.append(client_txns[client_txns["read"]]["duration"].to_numpy())
                write_latencies.append(client_txns[client_txns["write"]]["duration"].to_numpy())

            def _calc_percentile(arr_list, p, conversion):
                if not arr_list: return None
                combined = np.concatenate(arr_list)
                if len(combined) == 0: return None
                return np.percentile(combined / conversion, p)

            for p in percentiles:
                latency_percentiles[f"all_p{p}"] = _calc_percentile(all_latencies, p, latency_units_conversion)
                latency_percentiles[f"lsh_p{p}"] = _calc_percentile(lsh_latencies, p, latency_units_conversion)
                latency_percentiles[f"fsh_p{p}"] = _calc_percentile(fsh_latencies, p, latency_units_conversion)
                latency_percentiles[f"mh_p{p}"] = _calc_percentile(mh_latencies, p, latency_units_conversion)
                latency_percentiles[f"read_p{p}"] = _calc_percentile(read_latencies, p, latency_units_conversion)
                latency_percentiles[f"write_p{p}"] = _calc_percentile(write_latencies, p, latency_units_conversion)
    ...
    for system in system_dirs:
        ...
        for x_val in x_vals:
            ...
            for ip in log_files[system.split('/')[-1]][x_val.split('/')[-1]]['resource_util_logs'].keys():
                if np.isnan(log_files[system.split('/')[-1]][x_val.split('/')[-1]]['resource_util_logs'][ip]['cpu_pct'].mean()):
                    continue
...
if scenario == 'vary_hw':
    ...
    df_main_metrics['x_var'] = df_main_metrics['x_var'].astype(cat_type)
    df_resource_util['x_var'] = df_resource_util['x_var'].astype(cat_type)
df_main_metrics = df_main_metrics.sort_values('x_var').round(3)
df_resource_util = df_resource_util.sort_values('x_var').round(3)
...
if latency_ablation:
    df_latency_ablations = df_latency_ablations.sort_values('x_var').round(3)
