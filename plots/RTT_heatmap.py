...
parser.add_argument('-sd', '--standard_deviation', default=False, action='store_true', help='whether to include standard deviation')
...
ax = sns.heatmap(
    ...
    vmin=0,
    vmax=250,
    xticklabels=True,  # Force all x labels to show
    yticklabels=True   # Force all y labels to show
)
...
plt.gca().set_xticklabels(data.columns, fontsize=8)
plt.gca().set_yticklabels(data.index, rotation=0, fontsize=8)  # Keep region labels readable
