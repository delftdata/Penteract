...
exec > /var/log/detock-setup.log 2>&1  # <--- LOG EVERYTHING TO THIS FILE
export DEBIAN_FRONTEND=noninteractive

# 1. FORCE IPv4 (Fixes the TimeoutError)
echo 'Acquire::ForceIPv4 "true";' > /etc/apt/apt.conf.d/99force-ipv4

# 2. RETRY FUNCTION (Essential for flaky AWS-to-PPA connections)
function retry {{
  local n=1
  local max=10
  local delay=10
...
echo "Starting Detock Setup at $(date)"
...
# --- RESILIENT REPO ADDITION ---
retry apt-get update -qq
retry apt-get install -y software-properties-common
# Use the -y flag and force IPv4 for the PPA addition
retry add-apt-repository -y ppa:deadsnakes/ppa
retry apt-get update -qq

# --- INSTALL PACKAGES (Added retry) ---
retry apt-get install -y -qq build-essential libssl-dev zlib1g-dev libbz2-dev \
...
retry apt-get install -y -qq python3-openssl git zip python3.8 python3.8-venv python3.8-dev
...
# We use absolute paths to ensure Python 3.8 is found
...
# Python Environment - USE ABSOLUTE PATH FOR 3.8
/usr/bin/python3.8 -m venv /home/ubuntu/build_detock
