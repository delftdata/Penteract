...
2. SSH into one of your clients, e.g., `ssh -i keys/my_aws_key_us-west-1.pem -o StrictHostKeyChecking=no ubuntu@54.219.144.100`
3. Activate environment `source build_detock/bin/activate && cd Detock/`
4. Spawn the DB service (from within one of your AWS client VMs): `python3 tools/admin.py start --image omraz/seq_eval:latest aws/conf_files/tpcc/aws_tpcc_calvin.conf -u ubuntu -e GLOG_v=1 --bin slog`
5. Test DB service status `python3 tools/admin.py status --image omraz/seq_eval:latest aws/conf_files/ycsb/aws_ycsb_ddr_ts.conf -u ubuntu`
...
7. Run a whole scenario `python3 tools/run_config_on_remote.py -i omraz/seq_eval:latest -m ubuntu@18.145.67.119 -s network -w tpcc -c aws/conf_files/tpcc/aws_tpcc_calvin.conf -u ubuntu -bl True -db calvin 2>&1 | tee scenario_$(date +"%d-%m-%y_%H-%M-%S").log`
8. Run all systems `python3 tools/run_all_systems_on_remote.py -i omraz/seq_eval:latest -m ubuntu@50.18.17.51 -s baseline -w tpcc -cf aws/conf_files/ycsb -u ubuntu`
...
Use instructions from `crdb/README.md` to spawn up and populate a CRDB cluster.

To run an experiment with CRDB use the command:

`python3 tools/run_config_on_remote.py -i omraz/seq_eval:crdb_benchmark -m ubuntu@3.101.88.223 -s network -w ycsb -c aws/conf_files/ycsb/aws_ycsb_crdb.conf -u ubuntu -bl True -db crdb 2>&1 | tee scenario_$(date +"%d-%m-%y_%H-%M-%S").log`

And for TPC-C:

`python3 tools/run_config_on_remote.py -i omraz/seq_eval:crdb-custom -m ubuntu@3.101.88.223 -s network -w tpcc -c aws/conf_files/tpcc/aws_tpcc_crdb.conf -u ubuntu -bl True -db crdb 2>&1 | tee scenario_$(date +"%d-%m-%y_%H-%M-%S").log`
...
Helper command to remove netem conditions on AWS:
`sudo tc qdisc del dev ens3 root || true`