...
void Server::ProcessStatsRequest(const internal::StatsRequest& stats_request) {
  ...
  stats.AddMember(StringRef(NUM_PENDING_RESPONSES), static_cast<uint64_t>(pending_responses_.size()), alloc);
  stats.AddMember(StringRef(NUM_PARTIALLY_FINISHED_TXNS), static_cast<uint64_t>(finished_txns_.size()), alloc);
  if (level >= 1) {
