...
void DDRLockManager::GetStats(rapidjson::Document& stats, uint32_t level) const {
  ...
  stats.AddMember(StringRef(NUM_DEADLOCKS_RESOLVED), static_cast<uint64_t>(num_deadlocks_resolved_.load()), alloc);
  {
    lock_guard<SpinLatch> guard(txn_info_latch_);
    stats.AddMember(StringRef(NUM_TXNS_WAITING_FOR_LOCK), static_cast<uint64_t>(txn_info_.size()), alloc);
    if (level >= 1) {
