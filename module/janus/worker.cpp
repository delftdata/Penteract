...
Worker::Worker(int id, const std::shared_ptr<Broker>& broker, const std::shared_ptr<Storage>& storage,
  ...
  // How come we don't need this for MovR, DS Hotels, and DS Movie?
  switch (config()->execution_type()) {
    ...
    case slog::internal::ExecutionType::TPC_C:
      ...
      break;
    case slog::internal::ExecutionType::Bench_X:
      execution_ = make_unique<slog::BenchXExecution>(sharder_, storage);
      break;
    case slog::internal::ExecutionType::PPS:
      execution_ = make_unique<slog::PPSExecution>(sharder_, storage);
      break;
...
void Worker::OnInternalRequestReceived(EnvelopePtr&& env) {
  ...
  if (txn->status() != TransactionStatus::ABORTED) {
    if (read_result.will_abort()) {
      ...
      // Apply remote reads.
      if (config()->execution_type() == slog::internal::ExecutionType::PPS ||
          config()->execution_type() == slog::internal::ExecutionType::MOVIE ||
          config()->execution_type() == slog::internal::ExecutionType::small_bank) {
        // For PPS, since we access a key twice (read + update), we need to make sure that we don't duplicate it.
        for (const auto& kv : read_result.reads()) {
          const auto& incoming_key = kv.key();
          const auto& incoming_value = kv.value_entry().value();

          if (incoming_value.empty()) {
            continue;  // Skip empty values
          }

          bool found = false;

          for (auto& existing_kv : *txn->mutable_keys()) {
            if (existing_kv.key() == incoming_key) {
              existing_kv.mutable_value_entry()->set_value(incoming_value);
              found = true;
              break;
            }
          }
