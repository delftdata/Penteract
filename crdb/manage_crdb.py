...
def apply_tpcc_index_partitioning(primary_ip, regions_list, warehouse_count=TPCC_WAREHOUSE_COUNT, user=DEFAULT_USER, env='st', robust_mode=True, workload='tpcc'):
    ...
    if workload == 'benchx':
        index_targets.append(("history", "history_idx", "h_c_w_id"))
...
def populate_db_tables(all_ips, primary_ip, workload='ycsb', env='st', regions_list=['usw', 'euw'], robust_mode=True, user=DEFAULT_USER):
    ...
    if workload == 'ycsb':
    ...
    elif workload == 'tpcc' or workload == 'benchx':
        ...
        if workload == 'benchx':
            print("--- Action: Creating BenchX specific indexes ---")
            idx_sql = f"USE {TARGET_DB_NAME}; CREATE INDEX IF NOT EXISTS item_idx ON item (i_name); CREATE INDEX IF NOT EXISTS history_idx ON history (h_c_w_id, h_c_d_id, h_c_id, h_w_id, h_d_id);"
            idx_cmd = f"docker exec -i crdb-node ./cockroach sql --insecure --host={primary_ip} <<'EOF'\n{idx_sql}\nEOF"
            run_remote(primary_ip, idx_cmd, user=user)
        ...
        if partition_indexes:
            ...
            apply_tpcc_index_partitioning(primary_ip, regions_list, warehouse_count=TPCC_WAREHOUSE_COUNT, user=user, env=env, robust_mode=robust_mode, workload=workload)
    else:
...
def main():
    ...
    parser.add_argument('-w',  "--workload", default='ycsb', choices=['ycsb', 'tpcc', 'benchx'], help="Workload type for DB population")
...
memory = 128
    ...
    if action == "stop":
        ...
        if workload not in ['tpcc', 'benchx']:
            print("Partitioning logic is currently only implemented for TPC-C and BenchX. Please specify --workload tpcc or benchx to use this action.")
            return
