...
VALID_WORKLOADS = ['ycsb', 'tpcc', 'pps', 'smallbank', 'movr', 'movie', 'dsh', 'benchx']
...
def benchmark_ycsb(server_ips, image, user, clients, duration, params, seed, txns, workload, prime_region_ips, client_ips):
    ...
    for i, (client_ip, prime_region_ip) in enumerate(zip(client_ips, prime_region_ips)):
        ...
        docker_cmd = f"docker run -d --name crdb-client --net=host {image} {cpp_cmd}"
...
def benchmark_benchx(server_ips, image, user, clients, duration, params, seed, txns, workload, prime_region_ips, client_ips, scenario='scalability'):
    # 1. Parse BenchX specific parameters (similar to TPC-C)
    total_warehouses = 1200
    if 'warehouses=' in params:
        total_warehouses = int(params.split('warehouses=')[1].split(',')[0])
    ...
    for i, (client_ip, prime_region_ip) in enumerate(zip(client_ips, prime_region_ips)):
        ...
        # We pass the raw params string so the C++ tool can parse it
        benchx_cmd = f"./benchmark_benchx_crdb {prime_region_ip} {clients} {duration} {nparts} {nregs} {i} '{params}' {seed + i}"
        
        docker_cmd = f"docker run -d --name crdb-client --net=host {image} {benchx_cmd}"
    ...
    while not benchmark_complete:
        ...
        for client_ip in client_ips:
            result = run_remote(client_ip, "docker logs crdb-client", user)
            logs = result.stdout + result.stderr
            if "Total Committed:" not in logs and "Error: ":
                benchmark_complete = False
...
def main():
    ...
    parser.add_argument('-db', '--database', default='crdb', help="Database to use (ignored, included for compat)")
    parser.add_argument('--sample', default=100, help="Sampling rate")
    ...
    if action == 'benchmark':
        ...
        for i, client_ip in enumerate(client_ips):
            ...
            if workload in ('ycsb', 'benchx'):
                ...
                # Copy from remote host /tmp to local experiment directory
