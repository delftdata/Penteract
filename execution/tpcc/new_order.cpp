...
bool NewOrderTxn::Write() {
  ...
  for (const auto& l : a_ol_) {
    //LOG(INFO) << "Updating Stock for item_id=" << l.a_item_id->value
    //          << " at supply_w_id=" << l.a_supply_w_id->value
    ...
    //LOG(INFO) << "Inserting OrderLine for item_id=" << l.a_item_id->value
    //          << " with quantity=" << l.a_quantity->value
