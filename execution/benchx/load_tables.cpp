...
class PartitionedBenchXDataLoader {
 public:
  PartitionedBenchXDataLoader(const StorageAdapterPtr& storage_adapter, int from_w, int to_w, int step, int seed)
  ...
  static void LoadItem(const StorageAdapterPtr& storage_adapter, const std::vector<int>& rep_w) {
    ...
    Table<ItemByNameSchema> item_by_name(storage_adapter);
    ...
    for (int id = 1; id <= kMaxItems; id++) {
      std::string name = "ItemName_" + std::to_string(id);
      name.resize(24, ' ');
 ...
 private:
  ...
  void LoadCustomer(size_t &total_customer_size, size_t &total_history_size) {
    ...
    Table<CustomerByNameSchema> customer_by_name(storage_adapter_);
    ...
    for (int w_id = from_w_; w_id < to_w_; w_id += step_) {
      ...
      for (int d_id = 1; d_id <= kDistPerWare; d_id++) {
        for (int id = 1; id <= kCustPerDist; id++) {
          std::string string_34 = "CustomerName_" + std::to_string(w_id) + "_" + std::to_string(d_id) + "_" + std::to_string(id);
          string_34.resize(34, ' '); // Pad to 34 chars
...
void LoadTables(const StorageAdapterPtr& storage_adapter, int W, int num_regions, int num_partitions, int partition, int num_threads) {
  ...
  PartitionedBenchXDataLoader::LoadItem(storage_adapter, rep_w);

  std::atomic<int> num_done = 0;
  auto LoadFn = [&](int from_w, int to_w, int seed) {
    PartitionedBenchXDataLoader loader(storage_adapter, from_w, to_w, num_partitions, seed);
    loader.Load();
    num_done++;
  };
