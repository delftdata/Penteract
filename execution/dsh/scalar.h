...
NEW_PRIMITIVE_SCALAR(Float32);
NEW_PRIMITIVE_SCALAR(Float64);
...
struct VarTextScalar : public Scalar {
  VarTextScalar(const std::shared_ptr<DataType>& type, const std::string& buffer) : Scalar(type), buffer(buffer) {}
  VarTextScalar(const std::shared_ptr<DataType>& type, std::string&& buffer)
      : Scalar(type), buffer(std::move(buffer)) {}
...
using VarTextScalarPtr = std::shared_ptr<VarTextScalar>;


inline std::shared_ptr<VarTextScalar> MakeVarTextScalar(const std::shared_ptr<DataType>& type,
  const std::string& data) {
CHECK(type->name() == DataTypeName::VAR_TEXT);
CHECK_LE(data.size(), type->size()) << "Size is too large: \"" << data << "\". Need size <= " << type->size();
return std::make_shared<VarTextScalar>(type, data);
}

template <size_t Width>
inline std::shared_ptr<VarTextScalar> MakeVarTextScalar(const std::string& data) {
return MakeVarTextScalar(VarTextType<Width>::Get(), data);
}

inline std::shared_ptr<VarTextScalar> MakeVarTextScalar() {
return MakeVarTextScalar(VarTextType<0>::Get(), "");
}
...
inline ScalarPtr MakeScalar(const std::shared_ptr<DataType>& type, const void* data) {
  switch (type->name()) {
    ...
    case DataTypeName::F32:
      return MakeFloat32Scalar(*reinterpret_cast<const Float32Type::CType*>(data));
    case DataTypeName::F64:
      return MakeFloat64Scalar(*reinterpret_cast<const Float64Type::CType*>(data));
    case DataTypeName::FIXED_TEXT:
      return MakeFixedTextScalar(type, {static_cast<const char*>(data), type->size()});
    case DataTypeName::VAR_TEXT:
      return MakeVarTextScalar(type, {static_cast<const char*>(data), type->size()});
  }
  return nullptr;
}
...
inline bool operator==(const VarTextScalar& s1, const VarTextScalar& s2) { return s1.buffer == s2.buffer; }
...
inline bool operator==(const Scalar& s1, const Scalar& s2) {
  ...
  switch (s1.type->name()) {
    ...
    case DataTypeName::VAR_TEXT:
      return static_cast<const VarTextScalar&>(s1) == static_cast<const VarTextScalar&>(s2);
    case DataTypeName::F32:
      return static_cast<const Float32Scalar&>(s1) == static_cast<const Float32Scalar&>(s2);
    case DataTypeName::F64:
      return static_cast<const Float64Scalar&>(s1) == static_cast<const Float64Scalar&>(s2);
    }
  return false;
}
