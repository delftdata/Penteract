...
DEFINE_string(wl, "basic", "Name of the workload to use (options: basic, cockroach, remastering, tpcc, pps, morv, movie, dsh, smallbank, benchx)");
...
vector<unique_ptr<ModuleRunner>> InitializeGenerators() {
  ...
  for (int i = 0; i < FLAGS_generators; i++) {
    ...
    } else if (FLAGS_wl == "benchx") {
      workload = make_unique<BenchXWorkload>(config, FLAGS_region, FLAGS_replica, FLAGS_params, std::make_pair(i + 1, FLAGS_generators), seed + i);
    } else if (FLAGS_wl == "smallbank") {
      workload = make_unique<SmallBankWorkload>(config, FLAGS_region, FLAGS_replica, FLAGS_params, std::make_pair(i + 1, FLAGS_generators), seed + i);
    } else if (FLAGS_wl == "dsh") {
      workload = make_unique<DeathStarHotelWorkload>(config, FLAGS_region, FLAGS_replica, FLAGS_params, std::make_pair(i + 1, FLAGS_generators), seed + i);
    ...
    } else if (FLAGS_wl == "pps") {
      workload = make_unique<PPSWorkload>(config, FLAGS_region, FLAGS_replica, FLAGS_params, std::make_pair(i + 1, FLAGS_generators), seed + i);
    } else if (FLAGS_wl == "movie") {
      workload = make_unique<MovieWorkload>(config, FLAGS_region, FLAGS_replica, FLAGS_params, std::make_pair(i + 1, FLAGS_generators), seed + i);
    } else {
...
void RunBenchmark(vector<unique_ptr<ModuleRunner>>& generators) {
  ...
  for (;;) {
    ...
    // Effectively skip the first log since it is usually inaccurate. Then report: Send, Commit, Abort, Restart rates
    // (in brackets total numbers)
...
#ifdef __APPLE__
    std::this_thread::sleep_for(std::chrono::seconds(sigpoll_time.tv_sec) + std::chrono::nanoseconds(sigpoll_time.tv_nsec));
#else
...
struct ResultWriters {
  const vector<string> kTxnColumns = {"txn_id",   "coordinator",    "regions", "partitions",  "generator",
                                      "restarts", "global_log_pos", "sent_at", "received_at", "code"};
  const vector<string> kEventsColumns = {"txn_id", "event", "time", "machine", "home"};
  const vector<string> kSummaryColumns = {
      "committed",  "aborted",          "not_started",     "restarted", "single_home", "foreign_single_home",
      "multi_home", "single_partition", "multi_partition", "remaster",  "elapsed_time"};
...
CSVWriter& operator<<(CSVWriter& csv, const GeneratorSummary& s) {
  csv << s.committed << s.aborted << s.not_started << s.restarted << s.single_home << s.foreign_single_home
      << s.multi_home << s.single_partition << s.multi_partition << s.remaster;
  return csv;
}
...
void WriteResults(const vector<unique_ptr<ModuleRunner>>& generators) {
  ...
  for (const auto& info : txn_infos) {
    ...
    // Write the arguments that are sent to the server for each transaction.
    int code_length = info.txn->mutable_code()->procedures(0).args_size();
    std::stringstream code_str;
    for (int i = 0; i < code_length; i++) {
      const std::string& arg = info.txn->mutable_code()->procedures(0).args(i);
      // Safeguard, just in case that field in the protobuf is not set
      if (arg.empty()) {
          code_str << "<null>";  // or skip, or log, or error
      } else {
          std::string safe_arg = arg;
          std::replace(safe_arg.begin(), safe_arg.end(), ',', ';');
          code_str << safe_arg;
      }
      if (i != code_length - 1) {
          code_str << ";";
    ...
    /*for (int i = 0; i < code_length; i++) {
      code_str << info.txn->mutable_code()->procedures(0).args(i);
      if (i != code_length - 1) {
        code_str << ";";
      }
    }*/
    ...
    writers->txns << txn_internal.id() << txn_internal.coordinating_server() << involved_regions << involved_partitions
                  ...
                  << info.recv_at.time_since_epoch().count() << code_str.str() << csvendl;
