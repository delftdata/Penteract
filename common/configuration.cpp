...
Configuration::Configuration(const internal::Configuration& config, const string& local_address)
  ...
  if (config_.execution_type() == internal::ExecutionType::Bench_X) {
    CHECK(config_.has_benchx_partitioning()) << "BenchX execution type can only be paired with BenchX partitioning";
  }

  if (config_.has_benchx_partitioning()) {
    CHECK(config_.execution_type() == internal::ExecutionType::Bench_X)
        << "BenchX partitioning can only be paired with BenchX execution type";
  }

  if (config_.execution_type() == internal::ExecutionType::PPS) {
    CHECK(config_.has_pps_partitioning()) << "PPS execution type can only be paired with PPS partitioning";
  }

  if (config_.has_pps_partitioning()) {
    CHECK(config_.execution_type() == internal::ExecutionType::PPS)
        << "PPS partitioning can only be paired with PPS execution type";

    auto pps_partitioning = config_.pps_partitioning();

    // Treate Calvin's case separately, as it has a single region but potentially multiple replicas.
    CHECK(pps_partitioning.parts_per_product_max_regions() >= 1 &&
          pps_partitioning.parts_per_product_max_regions() <=
              (num_regions() == 1 ? num_replicas(local_region()) : num_regions()))
        << "Invalid maximum number of regions for parts per products within the PPS partitioning";

    CHECK(pps_partitioning.parts_per_product_max_partitions() >= 1 &&
          pps_partitioning.parts_per_product_max_partitions() <= config_.num_partitions())
        << "Invalid maximum number of partitions for parts per products within the PPS partitioning";
  }

  if (config_.execution_type() == internal::ExecutionType::small_bank) {
    CHECK(config_.has_smallbank_partitioning())
        << "SmallBank execution type can only be paired with SmallBank partitioning";
  }

  if (config_.has_smallbank_partitioning()) {
    CHECK(config_.execution_type() == internal::ExecutionType::small_bank)
