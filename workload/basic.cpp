  ...
  for (size_t i = 0; i < records; i++) {
    ...
    for (;;) {
      ...
      if (ins.second) {
        ...
        if (i < writes) {
          auto random_string = rnd_str_(value_size);
          code.push_back({"SET", key, random_string});
          keys.emplace_back(key, KeyType::WRITE);
          record.is_write = true;
          
          txn_string.append("SET").append(";")
                    .append(key).append(";")
        ...
        avg_txn_size = (txn_count * avg_txn_size + (float) total_bytes) / (txn_count+1);
        txn_count++;

        if (txn_count % 100000 == 0) {
          LOG(INFO) << "Current txn counts: Total: " << txn_count << ", " << avg_txn_size << " bytes on average";
        }
