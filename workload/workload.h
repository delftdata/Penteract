...
struct TransactionProfile {
  ...
  enum DependencyType { NONE, FIRST_PHASE, SECOND_PHASE, THIRD_PHASE };
  DependencyType dependency_type = NONE;

  enum TransactionType { NOTHING, BALANCE, DEPOSIT_CHECKING, TRANSACTION_SAVING, AMALGAMATE, WRITECHECK };
  TransactionType transaction_type = NOTHING;
...
class Workload {
 public:
  ...
  virtual void refreshSunflowerRegions(int64_t duration, int64_t elapsed_time) {
    LOG(INFO) << "No sunflower scenario available for this workload";
  }

  virtual bool HasDependentTxns() const { return false; }
