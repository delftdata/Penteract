...
TEST_TARGET="${TEST_TARGET:-consistency_multiregion_test}"

PROTOCOLS=(
    "detock:DDR:COUNTERLESS:AllProtocols/ConsistencyMultiRegionTest.RunsConsistentWorkload/Detock"
    "ddr_only:DDR:COUNTERLESS:AllProtocols/ConsistencyMultiRegionTest.RunsConsistentWorkload/DDROnly"
    "slog:DDR:COUNTERLESS:AllProtocols/ConsistencyMultiRegionTest.RunsConsistentWorkload/Slog"
    "calvin:DDR:COUNTERLESS:AllProtocols/ConsistencyMultiRegionTest.RunsConsistentWorkload/Calvin"
    "janus:DDR:COUNTERLESS:AllProtocols/ConsistencyMultiRegionTest.RunsConsistentWorkload/Janus"
)

OUTPUT_DIR="${OUTPUT_DIR:-acid_results}"
mkdir -p "${OUTPUT_DIR}"
RESULTS_CSV="${RESULTS_CSV:-${OUTPUT_DIR}/multiregion_consistency_results.csv}"
LOG_DIR="${LOG_DIR:-${OUTPUT_DIR}/multiregion_consistency_logs}"
...
for ENTRY in "${PROTOCOLS[@]}"; do
    ...
    echo "========================================"
    echo "Multi-region consistency — $SCOPE (LM=$LM, REMASTER=$REMASTER)"
    echo "========================================"

    TAG_LM=$(echo "$LM" | tr '[:upper:]' '[:lower:]')
    TAG_RM=$(echo "$REMASTER" | tr '[:upper:]' '[:lower:]')
    TAG="mr-${TAG_LM}-${TAG_RM}"
    ...
    if [ "${LOCAL_BUILD}" -eq 0 ]; then
        if ! docker build \
            -t detock-test:${TAG} \
            -f scripts/acid/Dockerfile.test.multiregion \
...
[ "${OVERALL_EXIT}" -ne 0 ] && echo "Multi-region consistency finished with failures." && exit "${OVERALL_EXIT}"
echo "Multi-region consistency complete. Results: ${RESULTS_CSV}"
