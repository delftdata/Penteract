...
TEST_TARGET="${TEST_TARGET:-isolation_level_singleregion_test}"

PROTOCOLS=(
    "detock:DDR:COUNTERLESS:AllProtocols/IsolationLevelSingleRegionTest.*/Detock"
    "ddr_only:DDR:COUNTERLESS:AllProtocols/IsolationLevelSingleRegionTest.*/DDROnly"
    "slog:DDR:COUNTERLESS:AllProtocols/IsolationLevelSingleRegionTest.*/Slog"
    "calvin:DDR:COUNTERLESS:AllProtocols/IsolationLevelSingleRegionTest.*/Calvin"
    "janus:DDR:COUNTERLESS:AllProtocols/IsolationLevelSingleRegionTest.*/Janus"
)

OUTPUT_DIR="${OUTPUT_DIR:-acid_results}"
mkdir -p "${OUTPUT_DIR}"
RESULTS_CSV="${RESULTS_CSV:-${OUTPUT_DIR}/singleregion_isolation_results.csv}"
LOG_DIR="${LOG_DIR:-${OUTPUT_DIR}/singleregion_isolation_logs}"
...
for ENTRY in "${PROTOCOLS[@]}"; do
    ...
    echo "========================================"
    echo "Single-region isolation level — $SCOPE (LM=$LM, REMASTER=$REMASTER)"
    echo "========================================"
...
[ "${OVERALL_EXIT}" -ne 0 ] && echo "Single-region isolation finished with failures." && exit "${OVERALL_EXIT}"
echo "Single-region isolation complete. Results: ${RESULTS_CSV}"
