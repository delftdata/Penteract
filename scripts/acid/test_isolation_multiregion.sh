...
TEST_TARGET="${TEST_TARGET:-isolation_level_multiregion_test}"

PROTOCOLS=(
    "detock:DDR:COUNTERLESS:AllProtocols/IsolationLevelMultiRegionTest.*/Detock"
    "ddr_only:DDR:COUNTERLESS:AllProtocols/IsolationLevelMultiRegionTest.*/DDROnly"
    "slog:DDR:COUNTERLESS:AllProtocols/IsolationLevelMultiRegionTest.*/Slog"
    "calvin:DDR:COUNTERLESS:AllProtocols/IsolationLevelMultiRegionTest.*/Calvin"
    "janus:DDR:COUNTERLESS:AllProtocols/IsolationLevelMultiRegionTest.*/Janus"
)

OUTPUT_DIR="${OUTPUT_DIR:-acid_results}"
mkdir -p "${OUTPUT_DIR}"
RESULTS_CSV="${RESULTS_CSV:-${OUTPUT_DIR}/multiregion_isolation_results.csv}"
LOG_DIR="${LOG_DIR:-${OUTPUT_DIR}/multiregion_isolation_logs}"
...
for ENTRY in "${PROTOCOLS[@]}"; do
    ...
    echo "========================================"
    echo "Multi-region isolation level — $SCOPE (LM=$LM, REMASTER=$REMASTER)"
    echo "========================================"

    TAG_LM=$(echo "$LM" | tr '[:upper:]' '[:lower:]')
    TAG_RM=$(echo "$REMASTER" | tr '[:upper:]' '[:lower:]')
    TAG="mr-iso-${TAG_LM}-${TAG_RM}"
...
[ "${OVERALL_EXIT}" -ne 0 ] && echo "Multi-region isolation finished with failures." && exit "${OVERALL_EXIT}"
echo "Multi-region isolation complete. Results: ${RESULTS_CSV}"
